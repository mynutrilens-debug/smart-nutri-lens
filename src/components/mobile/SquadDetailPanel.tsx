...
import {
  Copy, Share2, Trophy, Flame, Crown, Medal, Award, LogOut, Loader2,
...
export function SquadDetailPanel({ squadId, onLeft, onDeleted }: Props) {
  ...
  const leaveMut = useMutation({
    ...
    onSuccess: () => {
      ...
      if (onLeft) onLeft(); else navigate({ to: "/squads" });
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["squads"] });
      if (onDeleted) onDeleted(); else navigate({ to: "/squads" });
    },
    onError: (e: any) => toast.error(e.message),
  });
  ...
  const memberIds = useMemo(() => leaderboard.map((l: any) => l.user_id).join(","), [leaderboard]);
  useEffect(() => {
  ...
  if (isLoading || !squad) {
    return <div className="flex items-center justify-center py-10"><Loader2 className="h-5 w-5 animate-spin text-emerald-300" /></div>;
  }
    ...
    <div className="space-y-4 pt-1">
      {isOwner && (
        <div className="flex items-center justify-between px-1">
          <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-[0.22em] text-emerald-300"><Radio className="h-3 w-3 animate-pulse" /> Live</span>
          {!editing && (
            <button onClick={() => setEditing(true)} className="h-8 px-3 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center gap-1.5 text-[11px]">
              <Pencil className="h-3 w-3 text-emerald-300" /> Edit
            </button>
          )}
        </div>
      )}
      ...
      {isOwner && editing && (
        <section className="rounded-2xl border border-emerald-400/25 bg-white/[0.03] backdrop-blur-xl p-4 space-y-3 animate-fade-in">
          <div>
            <label className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Squad name</label>
            <input value={editName} onChange={(e) => setEditName(e.target.value)} maxLength={60} className="mt-1 w-full rounded-xl bg-white/[0.04] border border-white/[0.06] px-3 py-2 text-sm focus:outline-none focus:border-emerald-400/40" />
          </div>
          <div>
            <label className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Goal description</label>
            <input value={editGoal} onChange={(e) => setEditGoal(e.target.value)} maxLength={200} placeholder="e.g. Lose 2kg together" className="mt-1 w-full rounded-xl bg-white/[0.04] border border-white/[0.06] px-3 py-2 text-sm focus:outline-none focus:border-emerald-400/40" />
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button onClick={() => setEditing(false)} className="rounded-xl py-2 border border-white/[0.06] bg-white/[0.03] text-sm flex items-center justify-center gap-1.5"><X className="h-4 w-4" /> Cancel</button>
            <button onClick={() => editMut.mutate()} disabled={editMut.isPending || editName.trim().length < 2} className="rounded-xl py-2 text-sm font-semibold bg-gradient-to-r from-emerald-400 to-cyan-400 text-black flex items-center justify-center gap-1.5 disabled:opacity-50"><Check className="h-4 w-4" /> {editMut.isPending ? "Saving…" : "Save"}</button>
          </div>
        </section>
      )}

      <section className="relative rounded-2xl border border-emerald-400/20 bg-gradient-to-br from-emerald-500/[0.08] via-white/[0.02] to-cyan-500/[0.06] backdrop-blur-xl p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Invite code</p>
            <p className="text-2xl font-mono font-bold tracking-widest text-emerald-200 mt-1">{squad.code}</p>
          </div>
          <div className="flex gap-2">
            <button onClick={copyCode} className="h-9 w-9 rounded-xl bg-white/[0.06] border border-white/[0.06] flex items-center justify-center"><Copy className="h-4 w-4" /></button>
            <button onClick={doShare} className="h-9 w-9 rounded-xl bg-gradient-to-br from-emerald-400 to-cyan-400 flex items-center justify-center shadow-[0_0_18px_rgba(52,211,153,0.4)]"><Share2 className="h-4 w-4 text-black" /></button>
          </div>
        </div>
      ...
      {me && (
        <section className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-xl p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">Your rank</p>
              <p className="text-2xl font-bold mt-0.5">#{leaderboard.findIndex((l: any) => l.user_id === me.user_id) + 1} <span className="text-sm text-muted-foreground font-normal">of {leaderboard.length}</span></p>
            </div>
          ...
          <div className="mt-3 grid grid-cols-4 gap-2">
            {[
              ...
              <div key={s.label} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-2 text-center">
      ...
      {finalized && rewards.length > 0 && (
        <section className="rounded-2xl border border-amber-400/20 bg-gradient-to-br from-amber-500/[0.1] via-white/[0.02] to-orange-500/[0.06] backdrop-blur-xl p-4">
          <div className="flex items-center gap-2 mb-3"><Trophy className="h-4 w-4 text-amber-300" /><h3 className="text-sm font-semibold">Podium & prizes</h3></div>
          <div className="space-y-2">
            {[...rewards].sort((a: any, b: any) => a.rank - b.rank).map((r: any) => {
              const winner = leaderboard.find((l: any) => l.user_id === r.user_id);
              return (
                <div key={r.id} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-white/[0.04] p-3">
                  <div className="h-9 w-9 rounded-lg bg-gradient-to-br from-amber-400 to-orange-400 flex items-center justify-center font-bold text-black">#{r.rank}</div>
                  ...
                  {r.coupon_code && (
                    <button onClick={() => { navigator.clipboard.writeText(r.coupon_code); toast.success("Coupon copied"); }} className="rounded-lg bg-white/[0.06] border border-white/[0.06] px-2.5 py-1.5 text-[10px] font-mono">{r.coupon_code}</button>
                  )}
                </div>
              );
            })}
        ...
        {leaderboard.length === 0 ? (
          ...
          <div className="space-y-2">
            {leaderboard.map((row: any, i: number) => {
              ...
              return (
                <div key={row.user_id} className={`flex items-center gap-3 rounded-xl border p-3 backdrop-blur-xl transition-all ${
                  isMe ? "border-emerald-400/40 bg-emerald-500/[0.08] shadow-[0_0_20px_rgba(52,211,153,0.15)]" : "border-white/[0.06] bg-white/[0.03]"
                }`}>
                  <div className={`h-9 w-9 rounded-lg flex items-center justify-center font-bold tabular-nums ${
          ...
          confirmDelete ? (
            <div className="rounded-2xl border border-rose-400/30 bg-rose-500/[0.08] p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button onClick={() => deleteMut.mutate()} disabled={deleteMut.isPending} className="rounded-xl py-2 text-xs font-semibold bg-rose-500 text-white disabled:opacity-50">{deleteMut.isPending ? "Deleting…" : "Delete squad"}</button>
              </div>
            </div>
          ) : (
            <button onClick={() => setConfirmDelete(true)} className="w-full rounded-2xl border border-rose-400/20 bg-rose-500/[0.06] text-rose-300 py-3 text-sm font-medium flex items-center justify-center gap-2"><Trash2 className="h-4 w-4" /> Delete squad (owner)</button>
          )
        ) : (
          <button onClick={() => leaveMut.mutate()} disabled={leaveMut.isPending} className="w-full rounded-2xl border border-rose-400/20 bg-rose-500/[0.06] text-rose-300 py-3 text-sm font-medium flex items-center justify-center gap-2"><LogOut className="h-4 w-4" /> {leaveMut.isPending ? "Leaving…" : "Leave squad"}</button>
        )}
      </section>
    </div>
  );
