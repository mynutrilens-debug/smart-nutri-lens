...
import { getSquadLeaderboard, finalizeSquad, leaveSquad } from "@/lib/squad.functions";
import { ArrowLeft, Copy, Share2, Trophy, Flame, Crown, Medal, Award, LogOut, Loader2, Sparkles, Users } from "lucide-react";
...
function SquadDetail() {
  ...
  // Auto-finalize on view if ended and no rewards yet
  useEffect(() => {
    ...
    <div className="px-5 pt-12 pb-28 space-y-5 relative">
      ...
      <header className="flex items-center gap-3 animate-slide-up">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground capitalize">{squad.period} · {squad.challenge_type.replace("_", " ")}</p>
      ...
      <section className="animate-slide-up" style={{ animationDelay: ".2s" }}>
        <div className="flex items-center justify-between px-1 mb-2">
          ...
          <span className="text-[10px] uppercase tracking-wider text-muted-foreground">Live · updates every 30s</span>
        </div>
            ...
            {leaderboard.map((row, i) => {
              ...
              return (
                <div key={row.user_id} className={`flex items-center gap-3 rounded-2xl border p-3.5 backdrop-blur-xl transition-all ${
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate">{row.display_name}{isMe && <span className="text-[10px] ml-1.5 text-emerald-300">(you)</span>}</p>
